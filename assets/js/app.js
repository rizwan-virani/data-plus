/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `DATAPLUS` namespace with
...
(function () {
  ...
  var S = window.DATAPLUS || {};
  var EX = (S && S.exam) || {};            // single source of truth for exam figures
  var PASS = EX.passing;                    // passing scaled score (from DATAPLUS.exam)
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("dataplus." + k, fb); },
      set: function (k, v) { write("dataplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("dataplus." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the DA0-002 is scored and delivered, and where the credential fits in a data analytics career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the DA0-002 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
      foot: EX.minutes + " min · ≤" + EX.maxQuestions + " items · " + PASS + " to pass", cta: "Read",
      onClick: function () { openExamMechanics(); }
    }));
    g1.appendChild(card({
      icon: "🧭", tag: "CAREER", title: "Career Guidance", tagCls: "ghost",
      desc: "Where Data+ sits on the career ladder: the vendor-neutral data-analytics fundamentals it certifies, and the data analyst, BI analyst, and reporting/business analyst roles it unlocks.",
      foot: "Foundational · Vendor-neutral", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Data+ acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the DA0-002 &mdash; " + EX.maxQuestions + " questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: Professor Messer's full DA0-002 course, the official CompTIA objectives PDF, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.DATAPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. Data+" });
  ...
  function openLabSandbox(lab, i) {
    ...
    function runCommand(raw) {
      ...
      if (low === "whoami") { logLine("student (Data Analyst)"); return; }
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates DATAPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.DATAPLUS_VER ? "?v=" + window.DATAPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "DA0-002 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: DA0-002", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Data+ fits in your path", S.careerGuidance || [])); }
  ...
  /* ===========================================================================
     BOOT
     =========================================================================== */
  /* Populate the hero exam-line from DATAPLUS.exam so the figures live in exactly
  ...
  function fillExamLine() {
    ...
    elx.innerHTML =
      ...
      "<span>❓ <b>up to " + EX.maxQuestions + " questions</b></span>" +
